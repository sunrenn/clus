...
let style = {
  ...
  function matchColor(range=[20,90]){
    ...
    console.log(typeof((123).toFixed(2)));
  ...
  cvsSize = [2160,3800];
//   cvsSize = [1024,1024];
  cvsSize = [cvsSize[0]*cvsScale,cvsSize[1]*cvsScale];
  ...
  let soundVisual = {
    ...
    svGetVal: function(){
      this.micLevel = this.mmiicc.getLevel()-0.05;
      return this.micLevel*20;
    }
  };
  ...
  function drawaline(x,y,nn,val=0){
    gMid.stroke(colorMid[0]+nn*val,colorMid[1],colorMid[2],colorMid[3]);
    gMid.strokeWeight(2);
    gMid.line(x-10,y-10,x+10,y+10);
  }
  function colorSquare(val){
    ...
    for (let ii=0;ii<stepNum[0]; ii++){
      for (let jj=0;jj<stepNum[1]; jj++){
        ...
        gFront.stroke(colorFront[0]+nn*val,colorFront[1],colorFront[2],colorFront[2]);
        gFront.strokeWeight(300+val*50-15*nn);
        gFront.point(...cvsCenterXY);
      }
    }
  }
  ...
  function say(){
    ...
    for (let ii=0;ii<txtlen;ii++) {
      gMid.noStroke();
      gMid.fill(...colorMid);
      gMid.text(txtarr[ii],(ii+0.5)*width/txtlen,frameCount%height);
    }
  }